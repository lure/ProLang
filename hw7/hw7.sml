...
fun shift(dx, dy, e) =
	...
	NoPoints => e
      ...
      | _ => raise BadProgram "shift received unoperable expression"
...
fun preprocess_prog e =
	...
	LineSegment (x1, y1, x2, y2) => if real_close_point (x1, y1)(x2, y2)
					then Point(x1, y1)
					    ...
					    if ((x1 - x2) > epsilon) orelse (real_close(x1, x2) andalso (y1 > y2))
      ...
      | Shift (x1, y1, e1) => Shift(x1, y1, preprocess_prog(e1))
      | _ => e
