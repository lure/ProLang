...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close_point(@x1, @y1, @x2, @y2)
      ...
      if ((@x1 - @x2) > Epsilon) || (real_close(@x1, @x2) && (y1 > y2))
  ...
  def intersectWithSegmentAsLineResult seg
    # seg2 = self, seg = seg ?
    x1start = seg.x1; y1start = seg.y1; x1end= seg.x2; y1end=seg.y2
    x2start = @x1; y2start = @y1; x2end = @x2; y2end = @y2
...
class Let < GeometryExpression
  ...
  def eval_prog env
    ...
    arr = Array.new(env)
    arr.insert(0, [@s, (@e1.eval_prog env)])
    @e2.eval_prog arr
  end
end
...
class Var < GeometryExpression
  ...
  def eval_prog env
    ...
    if i
      ...
      raise Exception.new 'Can\'t find requested variable'
    end
  end
end
